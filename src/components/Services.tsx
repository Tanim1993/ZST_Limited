...
export default function Services() {
  ...
  return (
    <section id="services" className="py-24 bg-slate-50 dark:bg-[#0A0F1D] border-t border-slate-100 dark:border-slate-900/60 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <h2 className="text-3xl md:text-5xl font-extrabold text-slate-950 dark:text-white tracking-tight mb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            {services.map((serv) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-200/50 dark:border-slate-800/50 space-y-1.5">
                    {serv.details.map((det, idx) => (
                      <span key={idx} className="flex items-center gap-2 text-[10px] text-slate-500 font-medium">
          ...
          <div className="lg:col-span-5 relative">
            <div className="sticky top-28 bg-white dark:bg-[#0D1527] border border-slate-250 dark:border-slate-800 rounded-2xl p-6 shadow-xl transition-all duration-300">
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      >
                        ...
                        <span className="text-[8px] text-slate-400 mt-1">{t.note}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="bg-slate-50 dark:bg-[#0A0F1D] p-4 rounded-xl border border-slate-100 dark:border-slate-900/60 space-y-3">
                  <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block">
                    Configured Solutions Core ({selectedServices.length})
                  </span>
                  
                  {selectedServices.length === 0 ? (
                    <p className="text-xs text-slate-500 italic">No services selected. Click elements on the left menu.</p>
                  ) : (
                    <div className="flex flex-wrap gap-1.5 max-h-[100px] overflow-y-auto pr-1">
                      {selectedServices.map((id) => {
                        const name = services.find(s => s.id === id)?.title.split(" ")[0] || id;
                        return (
                          <span key={id} className="text-[10px] bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 font-bold px-2 py-0.5 rounded flex items-center gap-1">
                            <CheckCircle className="w-2.5 h-2.5 text-emerald-500" /> {name}
                          </span>
                        );
                      })}
                    </div>
                  ...
                  <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-xs text-slate-800 dark:text-slate-200">
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase text-slate-400 font-bold">Projected Timeline</span>
                    <span className="text-lg md:text-xl font-bold font-mono text-slate-900 dark:text-white block">
                      {getEstimatedTimeline()}
                    </span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase text-slate-400 font-bold">Estimated Cost (USD)</span>
                    <span className="text-lg md:text-xl font-bold font-mono text-blue-600 dark:text-blue-400 block">
                ...
                <form onSubmit={handleEstimateQuerySubmit} className="space-y-2 border-t border-slate-200 dark:border-slate-800 pt-4">
                  <div>
                    <input 
                      type="text" 
                      required
                  ...
                  >
                    Generate Custom SLA Blueprint Proposal <ChevronRight className="w-3.5 h-3.5" />
                  </button>
                </form>

                {submitSuccess && (
                  <p className="text-[10px] text-emerald-500 font-medium text-center bg-emerald-50 dark:bg-emerald-950/40 p-2 rounded-lg border border-emerald-100 dark:border-emerald-900/60 animate-fade-in">
